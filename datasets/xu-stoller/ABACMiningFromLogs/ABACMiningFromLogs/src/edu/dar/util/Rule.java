...
public class Rule {
	...
	private HashSet<Triple<String, String, String>> coveredLogTriple;
	...
	public Rule(Rule r) {
		...
		this.coveredUPTriple = new HashSet<Triple<String, String, String>>(
				r.getCoveredUPTriple());
		if (r.getCoveredLogTriple() != null) {
			this.coveredLogTriple = new HashSet<Triple<String, String, String>>(r.getCoveredLogTriple());
		}
	...
	public String toString() {
		...
		return "rule(" + this.name + ";" + removeDelimiters(uae.toString())
	...
	public int getMaxUAESize() {
		...
		for (AttrValConjunct c : this.getUAE()) {
			if (c.getRHS() != null) {
				maxSize = maxSize >= c.getRHS().size() ? maxSize : c.getRHS()
						.size();
			}
			if (c.getRHSet() != null) {
				int size = 0;
	...
	public double getSize() {
		return this.getConjunctSize() + this.getCon().size()
				+ this.getOps().size();
	}
	...
	public HashSet<Triple<String, String, String>> getCoveredLogTriple() {
		return coveredLogTriple;
	}

	public void setCoveredLogTriple(
			HashSet<Triple<String, String, String>> coveredLogTriple) {
		this.coveredLogTriple = coveredLogTriple;
	}
}
