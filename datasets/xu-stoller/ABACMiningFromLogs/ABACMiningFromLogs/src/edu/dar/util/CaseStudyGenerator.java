...
class UniversityCaseStudyGenerator extends CaseStudyGenerator {
	...
	public void generateCaseStudy(String outputFile, int inputSize,
		...
		try {
			...
			out.write("rule(rule1; ; type in {gradebook}; {readMyScores}; crsTaken ] crs)\n");

			out.write("rule(rule2; ; type in {gradebook}; {addScore readScore}; crsTaught ] crs;)\n");

			out.write("rule(rule3; position in {faculty}; type in {gradebook}; {changeScore assignGrade}; crsTaught ] crs)\n");

			out.write("rule(rule4; department in {registrar}; type in {roster}; {read write}; )\n");

			out.write("rule(rule5; position in {faculty}; type in {roster}; {read}; crsTaught ] crs)\n");

			out.write("rule(rule6; ; type in {transcript}; {read}; uid=student)\n");

			out.write("rule(rule7; isChair in {true}; type in {transcript}; {read}; department=department)\n");

			out.write("rule(rule8; department in {registrar}; type in {transcript}; {read}; )\n");

			out.write("rule(rule9; ; type in {application}; {checkStatus}; uid=student)\n");

			out.write("rule(rule10; department in {admissions}; type in {application}; {read setStatus}; )\n");
	...
	public static void main(String[] args) {
		...
		// for (int i = 2; i <= Integer.parseInt(args[1]); i++) {
		// for (int j = 1; j <= 5; j++) {
		// if (i >= 10) {
		// c.generateCaseStudy(args[0] + "_" + i + "_" + j + ".abac",
		// i, Integer.parseInt(args[2]), 30);
		// } else {
		// c.generateCaseStudy(args[0] + "_0" + i + "_" + j + ".abac",
		// i, Integer.parseInt(args[2]), 30);
		// }
		// }
		// }
...
class HealthcareCaseStudyGenerator extends CaseStudyGenerator {
	...
	public void generateCaseStudy(String outputFile, int NWARD,
		...
		try {
			...
			out.write("rule(rule1; position in {nurse}; type in {HR}; {addItem}; ward=ward)\n");

			out.write("rule(rule2; ; type in {HR}; {addItem}; teams ] treatingTeam))\n");

			out.write("rule(rule3; ; type in {HR}; {addNote}; uid=patient)\n");

			out.write("rule(rule4; ; type in {HR}; {addNote}; agentFor ] patient)\n");

			out.write("rule(rule5; ; type in {HRitem}; {read}; uid=author)\n");

			out.write("rule(rule6; position in {nurse}; type in {HRitem}, topics supseteqln {{nursing}}; {read}; ward=ward)\n");

			out.write("rule(rule7; ; type in {HRitem}; {read}; specialties > topics, teams ] treatingTeam)\n");

			out.write("rule(rule8; ; type in {HRitem}, topics supseteqln {{note}}; {read}; uid=patient)\n");

			out.write("rule(rule9; ; type in {HRitem}, topics supseteqln {{note}}; {read}; agentFor ] patient)\n");
...
class ProjectManagementCaseStudyGenerator extends CaseStudyGenerator {
	...
	public void generateCaseStudy(String outputFile, int NDEPT,
		...
		try {
			...
			out.write("rule((rule1; adminRoles supseteqln {{manager}} ; type in {budget}; {read approve}; department = department)\n");

			out.write("rule((rule2; ; type in {schedule budget}; {read write}; projectsLed ] project)\n");

			out.write("rule((rule3; ; type in {schedule}; {read}; projects ] project)\n");

			out.write("rule((rule4; ; type in {task}; {setStatus}; tasks ] rid)\n");

			out.write("rule((rule5; ; type in {task}, proprietary in {false}; {request read}; projects ] project, expertise > expertise)\n");

			out.write("rule((rule6; isEmployee in {True} ; type in {task}; {request read}; projects ] project, expertise > expertise)\n");

			out.write("rule((rule7; adminRoles supseteqln {{auditor}} ; type in {budget}; {read}; projects ] project)\n");

			out.write("rule((rule8; adminRoles supseteqln {{accountant}} ; type in {budget}; {read write}; projects ] project)\n");

			out.write("rule((rule9; adminRoles supseteqln {{accountant}} ; type in {task}; {setCost}; projects ] project)\n");

			out.write("rule((rule10; adminRoles supseteqln {{planner}} ; type in {schedule}; {write}; projects ] project)\n");

			out.write("rule((rule11; adminRoles supseteqln {{planner}} ; type in {task}; {setSchedule}; projects ] project)\n");
