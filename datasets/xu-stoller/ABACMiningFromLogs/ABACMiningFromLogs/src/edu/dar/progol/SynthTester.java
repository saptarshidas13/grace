...
public class SynthTester 
{
	...
	public static final AbacFilter abacFilter=new AbacFilter();
	...
	public static void main(String[] args) 
	{
		if (args.length == 0) {
			...
			{
				if (args.length < 4) {
					System.err.println("Three arguments needed for -l mode. 1. inputABACbase (String) " +
							"2. inputLogBase (String)\n3. Output folder (String)");
					System.exit(1);
					
					//-l ./case-studies-synthetic/university_6 ./logs/case-studies-synthetic/university ./progol/case-studies-synthetic/university/
					//-l ./synthetic/_20 ./logs/synthetic/synthetic_20 ./progol/synthetic/
				}
				
				try 
				{
					
					abacFilter.setBase(args[1].substring(args[1].lastIndexOf('/')+1));
					args[1]=args[1].substring(0, args[1].lastIndexOf('/'));
					File abacDir=new File(args[1]);
					if (!abacDir.isDirectory())
					{
						prtrr("Error: arg[1] is not a directory");
						System.exit(1);
					}
					File[] abacs=abacDir.listFiles(abacFilter);
					if (abacs.length!=10)
					{
						if (abacs.length==0)
						{
							prtrr("Error: No files found");
							System.exit(1);
						}
						else if (abacs.length<10)
						{
							prtrr("Error: Missing files, only "+abacs.length+" were found.");
							System.exit(1);
						}
						else
						{
							prtrr("Warning: Over 10 .abac files found, only the first 10 will be processed.");
						}
					}
					
					
					String progDir=args[3]+abacFilter.getBase().substring(0, abacFilter.getBase().indexOf('_'));
					...
					for (int i=0; i<10; i++)
					{
						for (int j=6; j<=10; j++)
						{
							...
							File fl = new File(args[2]+"_"+j+"_"+i+".log");
							...
							Parser.parseInputABACFile(abacs[i].getAbsolutePath());
							...
							//prtr("Now generating "+progDir+'_'+i+'_'+j+"-prog.pl");
							//ProgolTranslator.translateLogToProgol(logs, progDir +'_'+i+'_'+j+"-prog.pl", Parser.config, attribVar);
							//Collections.shuffle(logs, new Random(System.currentTimeMillis()));
							//prtr("Now generating "+progDir+'_'+i+'_'+j+"-prog-shuf1.pl");
							//ProgolTranslator.translateLogToProgol(logs, progDir +'_'+i+'_'+j+"-prog-shuf1.pl", Parser.config, attribVar);
							//Collections.shuffle(logs, new Random(System.currentTimeMillis()));
							//prtr("Now generating "+progDir+'_'+i+'_'+j+"-prog-shuf2.pl");
							//ProgolTranslator.translateLogToProgol(logs, progDir +'_'+i+'_'+j+"-prog-shuf2.pl", Parser.config, attribVar);
							Collections.sort(logs);
							//prtr("Now generating "+progDir+'_'+i+'_'+j+"-prog-low.pl");
							//ProgolTranslator.translateLogToProgol(logs, progDir +'_'+i+'_'+j+"-prog-low.pl", Parser.config, attribVar);
							Collections.reverse(logs);
							prtr("Now generating "+progDir+'_'+i+'_'+j+"-prog-high.pl");
							ProgolTranslator.translateLogToProgol(logs, progDir +'_'+i+'_'+j+"-prog-high.pl", Parser.config, attribVar);
			...
			else if (args[0].charAt(1)=='q')
			{
				if (args.length < 4)
				{
					...
					//ex: -q ./synthetic/synthetic_0_20.abac ./progol/synthetic/progbac/#/ ./progol/synthetic/quality/synth0.output -high
				}
				
				//String nameBase=args[1].substring(args[1].lastIndexOf('/')+1, args[1].lastIndexOf('_'));
				String nameBase=args[1].substring(args[1].lastIndexOf('/')+1, args[1].indexOf('_'));
				...
				for (int completeness=6; completeness<=10; completeness++)
				{
					...
					//String progbac=args[2]+nameBase+"_"+completeness+order+".abac";
					String progbac=args[2]+nameBase+"_"+args[2].substring(args[2].lastIndexOf('/')-1, args[2].lastIndexOf('/'))+"_10_"+completeness+order+".abac";
					
					prtrr("Now processing "+progbac+"\t...\t");
					...
					if (args.length==6 || (args.length==5 && args[4].equals("-o")))
					{
						recordCoveredUP(coveredUP, Parser.config.getCoveredUP(), overAssignment, underAssignment, nameBase, 
								completeness, -1, overAssignmentRatio, underAssignmentRatio);
					}
			...
			{
				//1. path and base name for the file (./progol/synthetic/proglog/#/synthetic_#)
				//2. output path and name
				//3. sort order of the file (-high, -low, -shuf1, -shuf2)
				...
				try
				{
					...
					for (int j=6; j<=10; j++)
					{
						File f=new File(baseF+"_"+j+"-prog"+order+".proglog");
						if (!f.exists()) {
			...
			else if (args[0].charAt(1)=='a')
			{
				if (args.length<4)
				{
					...
					//ex: -a ./synthetic/synthetic_#_20.abac ./progol/synthetic/proglog/#/synthetic_ ./progol/synthetic/progbac/#/synthetic_# -high
				}
				...
				try
				{
					...
					String nameBase=args[2];
					
					for (int j=6; j<=10; j++)
					{
						f=new File(nameBase+nameBase.substring(nameBase.lastIndexOf('/')-1,nameBase.lastIndexOf('/'))+"_"+j+"-prog"+order+".proglog");
						fr=new FileReader(f);
						...
						f=new File(args[3]+"_"+j+order+".abac");
						if (!f.exists())
						{
							f.createNewFile();
						}
	...
	public static void prtrr(String s)
	{
		System.out.print(s);
	}
