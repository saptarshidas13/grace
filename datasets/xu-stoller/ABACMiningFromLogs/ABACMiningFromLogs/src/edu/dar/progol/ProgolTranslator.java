...
public class ProgolTranslator {
	...
	static ArrayList<String> assertion=new ArrayList<String>();
	...
	public static String translateConst(String v, boolean asrt) 
	{
		boolean changed=false;
		
		if (asrt==true && assertion.contains(v))	// && assertion.contains(v.toLowerCase()));
		{
			System.err.println("Error: Duplicate entry '"+v+"'");
			System.exit(1);
		}
		else if (asrt==true)
		{
		...
		if (v.contains("-"))
		{
			//System.err.println("Warning: '-' located in string, all '-' will be replaced with '_'");
			v=v.replaceAll("-", "_");
			changed=true;
		}
		try
		{
			Integer.parseInt(""+v.charAt(0));
			v="n"+v;
			changed=true;
			//System.err.println("Warning: String starts with a number, 'n' will be prepended");
		}
		...
		if (asrt==true && changed==true && assertion.contains(v))
		{
			System.err.println("Error: Duplicate entry '"+v+"'");
			System.exit(1);
		}
	...
	public static void translateABACToProgol(Config config, String outputFile, boolean attribVar) {
		try {
			...
			for (String user : config.getUsers()) {
				out.write("user(" + translateConst(user, true) + ").\n");
			}
			for (String resource : config.getResources()) {
				out.write("resource(" + translateConst(resource, true) + ").\n");
			}
			for (String operation : config.getOps()) {
				out.write("operation(" + translateConst(operation, true) + ").\n");
			}
			...
			for (String v : Val) {
				out.write("attribValAtomic(" + translateConst(v, false) + ").\n");
			}
			...
			for (String a : config.getUserAttrSet().keySet()) {
				...
				if (uAttr.getvType() == ValueType.Single) {
					for (String v : uAttr.getDomain()) {
						out.write(addType(translateUserAttrib(a)) + "(" + translateConst(v, false) + ").\n");
					}
				}
				if (uAttr.getvType() == ValueType.Set) {
					for (HashSet<String> vSet : uAttr.getSetDomain()) {
			...
			for (String a : config.getResourceAttrSet().keySet()) {
				...
				if (rAttr.getvType() == ValueType.Single) {
					for (String v : rAttr.getDomain()) {
						out.write(addType(translateResAttrib(a)) + "(" + translateConst(v, false) + ").\n");
					}
				}
				if (rAttr.getvType() == ValueType.Set) {
					for (HashSet<String> vSet : rAttr.getSetDomain()) {
				...
				for (String au : config.getUserAttrInfo().get(user).keySet()) {
					for (String val : config.getUserAttrInfo().get(user)
						...
						out.write(translateUserAttrib(au) + "("
								+ translateConst(user, false) + ","
								+ translateConst(val, false) + ").\n");
					}
				}
			}
...
//						}
			...
			for (Triple<String, String, String> up : upList) {
				if (config.getCoveredUP().contains(up)) {
					out.write("up(" + translateConst(up.getFirst(), false) + ","
							+ translateConst(up.getThird(), false) + ","
							+ translateConst(up.getSecond(), false) + ").\n");
				} else {
					out.write(":- up(" + translateConst(up.getFirst(), false) + ","
							+ translateConst(up.getThird(), false) + ","
							+ translateConst(up.getSecond(), false) + ").\n");
	...
	public static void translateLogToProgol(ArrayList<LogEntry> logs, String outputName, Config config, boolean attribVar)
	{
		try
		{
			...
			out.write("% Positive examples\n");
			for (LogEntry lge : logs)
			{
				out.write(translateConst(lge.toProgolString(), true));
			}
	...
	public static void flushAssertChecker()
	{
